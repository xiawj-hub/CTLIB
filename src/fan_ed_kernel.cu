            ...
            if (idxrow < *height) {
                ...
                while (idxi < *width && (idxd + dIndex0) < *dets && idxd < blockDim.x) {
                    if (detbound <= prebound) {
                        ...
                        threadprj += (pixbound - prebound) * image[idxchannel][0][idxrow][idxi] / (pixbound - prepixbound) * cweight(sourcex, sourcey, i0x - dImage / 2, i0y, s0);
                        prebound = pixbound;
                        idxi ++;                        
                        i0x += dImage;
                        prepixbound = pixbound;
                        ...
                        threadprj += (detbound - prebound) * image[idxchannel][0][idxrow][idxi] / (pixbound - prepixbound) * cweight(sourcex, sourcey, i0x - dImage / 2, i0y, s0);
                        prebound = detbound;
                        atomicAdd(prj+idxd, threadprj);
                        threadprj = 0;
                        idxd ++;
...
template <typename scalar_t>
__global__ void bprj_sv_fan_ed(
    torch::PackedTensorAccessor<scalar_t,4,torch::RestrictPtrTraits,size_t> image,
    const torch::PackedTensorAccessor<scalar_t,4,torch::RestrictPtrTraits,size_t> projection,
    const scalar_t* __restrict__ views, const scalar_t* __restrict__ dets,
    const scalar_t* __restrict__ width, const scalar_t* __restrict__ height,
            ...
            if (idxrow < *height) {
                ...
                while (idxi < *width && (idxd + dIndex0) < *dets && idxd < blockDim.x) {
                    if (detbound <= prebound) {
                        ...
                        prebound = pixbound;
                        threadprj *= cweight(sourcex, sourcey, i0x - dImage / 2, i0y, s0);
                        atomicAdd(&(image[idxchannel][idxview][idxrow][idxi]), threadprj);
                        threadprj = 0;
                        idxi ++;
                        i0x += dImage;
                        prepixbound = pixbound;
            ...
            if (idxcol < *width) {
                ...
                while (idxi < *height && (idxd + dIndex0) < *dets && idxd < blockDim.x) {
                    if (detbound <= prebound) {
                        ...
                        prebound = pixbound;
                        threadprj *= cweight(sourcex, sourcey, i0x, i0y - dImage / 2, s0);
                        atomicAdd(&(image[idxchannel][idxview][static_cast<int>(*height)-1-idxi][idxcol]), threadprj);
                        threadprj = 0;
                        idxi ++;
                        i0y += dImage;
                        prepixbound = pixbound;
...
torch::Tensor bprj_sv_fan_ed_cuda(torch::Tensor projection, torch::Tensor options) {
    cudaSetDevice(projection.device().index());
    auto views = options[0];
    auto dets = options[1];
    auto width = options[2];
    ...
    auto binshift = options[10];
    const int channels = static_cast<int>(projection.size(0));
    auto image = torch::zeros({channels, views.item<int>(), height.item<int>(), width.item<int>()}, projection.options());
    ...
    AT_DISPATCH_FLOATING_TYPES(projection.type(), "fan_beam_equal_distance_backprojection_single_view", ([&] {
        bprj_sv_fan_ed<scalar_t><<<blocks, BLOCK_DIM>>>(
            image.packed_accessor<scalar_t,4,torch::RestrictPtrTraits,size_t>(),
            projection.packed_accessor<scalar_t,4,torch::RestrictPtrTraits,size_t>(),
            views.data<scalar_t>(), dets.data<scalar_t>(), width.data<scalar_t>(),
            height.data<scalar_t>(), dImg.data<scalar_t>(), dDet.data<scalar_t>(),
